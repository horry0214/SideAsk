...
同时安装浏览器扩展和桌面端时，设置中的**网页中优先使用浏览器扩展**默认开启：Chrome、Edge、Firefox、Brave 等浏览器内只显示扩展的划词按钮，VS Code、PDF 阅读器、终端及其他原生应用由桌面端接管，因此不会出现两个解释按钮。`Alt+Shift+A` 始终可以强制使用桌面端；如果希望桌面按钮也出现在浏览器中，可以关闭该设置。
...
桌面端只发送用户主动选中的文字，以及当前小对话中的消息。“划词后显示解释按钮”默认关闭；开启后会在拖选或双击后通过 Windows UI Automation 读取已确认的文字选区，不使用自动 `Ctrl+C` 试探普通拖动，点击 **✦ 解释** 前也不会联系 Gateway 或 Provider。默认的浏览器优先路由只读取前台窗口所属的进程名，不读取网页内容。因为桌面端运行在另一个原生应用之外，所以不会读取选区周围的文档内容；浏览器插件仍可在网页内部携带受限的附近段落。
