...
4. Download `sideask-local-gateway-v0.7.0.zip` from the latest GitHub release, extract it, and run `npm start` with Node.js 20 or newer. The Gateway has no production dependencies to install and listens only on `127.0.0.1:8787`.
...
- Removing a Provider from the dashboard also removes it from the shared local Vault used by the desktop overlay.
